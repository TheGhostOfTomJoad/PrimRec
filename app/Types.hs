...
module Types (ATy (ATy), STy (..), Ctx, tyToSTy, DBIndex (..), STyContext (..), ADBIndex (ADBIndex), Eval, ValContext (..), lookupVar, tyToStyCPS, PR (..)) where
...
newtype PR n = PR {unPR :: ArithFun n}
...
tyToStyCPS (TyList n ty) g = SNat.fromNatCPS n (\sn -> tyToStyCPS ty (g . STyList sn))
...
lookupVar ZeroDB (a `ExtendVC` _) = a
lookupVar (SuccDB p) (_ `ExtendVC` b) = lookupVar p b
